...
def do_lambda_form(vals, env):
    ...
    if len(vals)>2:
        ...
        # Pair('lambda', Pair(formals,Pair(Pair('begin',vals.second),nil)))
    return LambdaProcedure(formals,body,env) 
    #Pair('lambda',Pair(formals,Pair(vals[1],nil)))
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        "*** YOUR CODE HERE ***"
        target = do_lambda_form(current_value)
    else:
